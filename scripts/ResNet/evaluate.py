...
if args.version == 'quant':
    if args.size == 18:
        model_paths = glob(f"../../models/quantized/Balanced/seed_{args.seed}/{args.task}_ResNET18*.pth")
        base = models.resnet18(weights=None)
        base.fc = nn.Linear(base.fc.in_features, 2)
    else:
        # model_paths = glob(f"../../models/quantized/{args.task}_ResNET50*.pth")
        model_paths = glob(f"../../models/quantized/Balanced/seed_{args.seed}/{args.task}_ResNET50*.pth")
    ...
    if args.size == 18:
        model_paths = glob(f"../../models/debiased/*_ResNET18*.pth")
        model = models.resnet18(weights=None)
        model.fc = nn.Linear(model.fc.in_features, 2)
    else:
        model_paths = glob(f"../../models/debiased/*_ResNET50*.pth")
        model = models.resnet50(weights=None)
        model.fc = nn.Linear(model.fc.in_features, 2)
...
if args.task == 'face':
    test_dataset = ImageFolder('../../data/images/face_test', transform=transform)
...
if args.version == 'quant':
    ...
    for path in tqdm(model_paths):
        debiased_m = copy.deepcopy(model)
        print(f'ON {path}')
        debiased_m.load_state_dict(torch.load(f"{path}"))
        debiased_m.to(device)
        predictions = []
        with torch.no_grad():
            for image, _ in test_loader:
                image = image.to(device)
                output = debiased_m(image)
                predicted = torch.argmax(output, dim=1).cpu().numpy()
                predictions.extend(predicted)
        pred_df = pd.DataFrame({"prediction": predictions})
        results = pd.concat([attributes, pred_df], axis=1)
